...
def get_files_to_commit() -> List[str]:
    ...
    for root, dirs, filenames in os.walk('.'):
        ...
        for filename in filenames:
            if not any(ignore in os.path.join(root, filename) for ignore in CUSTOM_IGNORE):
                files.append(os.path.join(root, filename))
    
    return files

def commit_changes(message: str = None) -> None:
...
if __name__ == "__main__":
    commit_message = input("Enter commit message (press Enter for auto-message): ").strip()
