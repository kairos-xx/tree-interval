...
def test_ast_node_access() -> None:
    ...
    found_node = tree.root.find(lambda n: n.info is not None and n.info.get("type") == "Assign")
...
def test_ast_node_fields() -> None:
    ...
    found_node = tree.root.find(lambda n: n.info is not None and n.info.get("type") == "FunctionDef")
    if not found_node or not found_node.ast_node:
        raise AssertionError("Node not found or ast_node is None")
