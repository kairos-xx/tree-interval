...
from ..core.interval_core import Leaf, Tree, LeafStyle
...
class TreeVisualizer:
    ...
    @staticmethod
    def visualize(tree: Tree[Any], config: Optional[VisualizationConfig] = None) -> None:
        ...
        def format_node_info(node: Leaf, level: int = 0, prefix: Optional[str] = None) -> str:
            parts: List[str] = []
            ...
            if config.show_info and node.info:
                ...
                if len(info_str) > remaining_width:
                    ...
                    parts.append(info_str if isinstance(node.info, dict)
                                else f"info={info_str}")
        ...
        def _print_node(node: Leaf, prefix: str = "", is_last: bool = True, 
                       level: int = 0) -> None:
